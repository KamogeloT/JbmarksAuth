...
const InlineSuccessScreen: React.FC<{ report: FaultReport; onNewReport: () => void }> = ({ report, onNewReport }) => {
  const [copied, setCopied] = useState(false);

  const handleCopy = async () => {
    try {
      await navigator.clipboard.writeText(report.refNumber);
      setCopied(true);
      setTimeout(() => setCopied(false), 2000);
    } catch {
      const el = document.createElement('textarea');
      el.value = report.refNumber;
  ...
  const handleShare = async () => {
    const text = `I've reported a ${report.formType} issue to JB Marks Municipality.\n\nReference: ${report.refNumber}\n\nTrack status in the JBMarks Community App.`;
    if (navigator.share) {
      try { await navigator.share({ title: 'Fault Report Submitted', text }); } catch { /* cancelled */ }
    } else {
  ...
  return (
    <div className="text-center p-6 animate-fade-in">
      ...
      <p className="text-gray-600 mb-6 text-sm">
        Your {report.formType.toLowerCase()} issue has been logged successfully.
      </p>
      
      <div className="bg-gray-50 border-2 border-dashed border-accent-DEFAULT rounded-xl p-5 mb-6">
        <p className="text-xs text-gray-500 font-semibold uppercase tracking-wider mb-2">
          Your Reference Number
        </p>
        <p className="text-2xl font-mono font-extrabold text-primary-dark tracking-wider">
          {report.refNumber}
        </p>
        {report.taskId && <p className="text-[10px] text-gray-400 mt-1">Task #{report.taskId}</p>}
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        <button
          ...
          className={`flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-bold border-2 transition-all ${
            copied ? 'bg-green-50 border-green-500 text-green-700' : 'bg-white border-gray-200 text-gray-700'
          }`}
        >
        ...
        <button
          onClick={handleShare}
          className="flex items-center justify-center gap-2 px-4 py-3 bg-white border-2 border-gray-200 rounded-xl text-sm font-bold text-gray-700"
        >
          📤 Share
        </button>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 mb-6">
        <p className="text-xs text-blue-800 leading-relaxed">
          💡 <strong>Keep this reference number.</strong> Use it in the <strong>Track</strong> tab to check your report status anytime.
        </p>
      </div>
      
      <button
        onClick={onNewReport}
        className="w-full py-3.5 px-4 text-white font-bold rounded-xl shadow-lg transition-all duration-300 border-2 border-primary-dark"
        style={{ backgroundColor: '#2E7D32' }}
      >
        ✓ Done — Report Another Issue
      </button>
    </div>
  );
};
    ...
    return (
      <div className="min-h-screen bg-light-DEFAULT">
        <div className="max-w-2xl mx-auto px-4 py-6">
          <InlineSuccessScreen report={successReport} onNewReport={handleNewReport} />
        </div>
      </div>
    );
  }
