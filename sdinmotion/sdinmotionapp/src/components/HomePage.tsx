...
interface HomePageProps {
  onNavigate: (view: 'report' | 'history' | 'track' | 'feed' | 'emergency' | 'announcements', faultType?: FaultType) => void;
}
    ...
    <div className="min-h-screen bg-secondary-DEFAULT pb-20">
      ...
      <div className="px-6 space-y-4">
        ...
        <button
          ...
          className="w-full text-white rounded-lg shadow-xl p-6 flex items-center justify-between transition-all duration-300 border-2 border-blue-800"
        ...
        >
          <div className="flex items-center space-x-4">
            ...
            <h3 className="font-extrabold text-lg text-white">Track My Report</h3>
          </div>
          <svg className="h-6 w-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
        </button>
      </div>
      ...
      <div className="px-6 mt-6">
        <h2 className="text-lg font-bold text-primary-dark mb-4 text-center">More</h2>
        <div className="grid grid-cols-2 gap-3 max-w-xs mx-auto">
          <button onClick={() => onNavigate('announcements')} className="flex flex-col items-center bg-white rounded-xl p-4 shadow-sm border border-gray-100">
            <span className="text-2xl mb-2">📢</span>
            <span className="text-xs font-semibold text-gray-700 text-center">Announcements</span>
          </button>
          <button onClick={() => onNavigate('emergency')} className="flex flex-col items-center bg-white rounded-xl p-4 shadow-sm border border-red-100">
            <span className="text-2xl mb-2">🚨</span>
            <span className="text-xs font-semibold text-red-700 text-center">Emergency</span>
          </button>
        </div>
      </div>
