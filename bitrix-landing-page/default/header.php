...
$asset->addCss(SITE_TEMPLATE_PATH . '/interface.css', true);
$asset->addCss(SITE_TEMPLATE_PATH . '/jbmarks_theme.css', true);
$asset->addJs(SITE_TEMPLATE_PATH . '/bitrix24.js', true);
...
if (str_contains($uri->getPath(), 'spaces'))
{
	function getListMode(): string
	{
			...
			?><table class="bx-layout-inner-table">
				<tr class="bx-layout-inner-top-row">
					...
					if ($isCompositeMode)
					{
						...
						if (str_contains($uri->getPath(), 'spaces'))
						{
							$spacesListMode = getListMode() === "collapsed" ? " --collapsed-mode" : "";
						}
