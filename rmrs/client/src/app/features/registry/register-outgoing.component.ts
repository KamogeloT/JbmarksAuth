...
/**
 * Component for registering outgoing records.
 * Captures required metadata fields for outgoing correspondence.
 */
@Component({
  selector: 'app-register-outgoing',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule],
  template: `
    <div class="register-form" aria-label="Register outgoing record form">
      <h2>Register Outgoing Record</h2>
      <p class="form-subtitle">Register a new outgoing correspondence with required metadata.</p>
      ...
      <form [formGroup]="form" (ngSubmit)="onSubmit()">
        <fieldset>
          ...
          <div class="form-row">
            <div class="form-group">
              <label for="senderOrRecipient">Recipient *</label>
              <input id="senderOrRecipient" type="text" formControlName="senderOrRecipient" placeholder="Recipient name or organization" aria-required="true"
            ...
            <div class="form-group">
              <label for="dateReceivedOrSent">Date Sent *</label>
              ...
              @if (isFieldInvalid('dateReceivedOrSent')) {
                <span class="error-text" role="alert">Date sent is required.</span>
              }
            </div>
          </div>
...
export class RegisterOutgoingComponent {
  ...
  onSubmit(): void {
    ...
    this.api.post<Record>('/records/outgoing', payload).subscribe({
      next: (record) => {
